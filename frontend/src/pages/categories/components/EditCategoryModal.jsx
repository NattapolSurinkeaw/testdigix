...
export default function EditCategoryModal({ open, category, onClose, onUpdated }) {
  ...
  useEffect(() => {
    if (category) {
      setForm({
        ...
        status_display: category.status_display ?? true,
      });
      setError('');
    }
  }, [category]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await categoryService.update(category.id, payload);
      if (res.status) {
        onClose();
  ...
  return (
    <Modal open={open} onClose={handleClose}>
      <Box sx={styleModal}>
        <h2 className="text-xl font-bold mb-4">แก้ไขหมวดหมู่</h2>
        {error && (
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-3">
          ...
          <div className="flex items-center gap-2">
            <input
              ...
              id="edit_cat_status"
            />
            <label htmlFor="edit_cat_status" className="text-sm">แสดงผล</label>
          </div>
